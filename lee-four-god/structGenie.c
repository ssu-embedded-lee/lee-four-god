...
int setDevice(const char *name_dev)		//디바이스명을 넣으면 해당 디바이스를 리스트에 넣어줌. 성공시 1 리턴
{
	...
	if(genie == NULL)
	{
		...
		genie->dev_state = DEVICE_OFF;
		strcpy(genie->dev_name,name_dev);
    	strcpy(genie->dev_token[0],"TurnOn");
    	strcpy(genie->dev_token[1],"TurnOff");
		return 1;
	}
	...
	cursor = cursor->next;
    cursor->dev_state = DEVICE_OFF;
    strcpy(cursor->dev_name,name_dev);
    strcpy(cursor->dev_token[0],"TurnOn");
    strcpy(cursor->dev_token[1],"TurnOff");
    return 1;
}
